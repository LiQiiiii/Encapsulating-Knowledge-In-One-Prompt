...
class CMISynthesizer(BaseSynthesis):
    def __init__(self, teacher, student, generator, nz, num_classes, img_size, 
                 feature_layers=None, cn = 1, bank_size=40960, n_neg=4096, head_dim=128, init_dataset=None,
    ...
    def synthesize(self, targets=None):
        ...
        best_aug = None
        best_inputs = None                #  256           256
        z = torch.randn(size=(self.synthesis_batch_size, self.nz), device=self.device).requires_grad_() 
        if targets is None:
            targets = torch.randint(low=0, high=self.num_classes, size=(self.synthesis_batch_size,)) # 256x1
            targets = targets.sort()[0] # sort for better visualization
        # print("targes: ", targets.size())
        ...
        self.student.train()
        self.data_pool.add( best_inputs, self.cn )
        ...
        return {"synthetic": best_inputs}, best_aug, targets
