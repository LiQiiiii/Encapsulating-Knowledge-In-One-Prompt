...
def save_image_batch_gray(imgs, output, col=None, size=None, pack=True):
    if isinstance(imgs, torch.Tensor):
        imgs = (imgs.detach().clamp(0, 1).cpu().numpy()*255).astype('uint8')
    base_dir = os.path.dirname(output)
    if base_dir!='':
    ...
    if pack:
        ...
        output_filename = output.strip('.png')
        for idx, img in enumerate(imgs):
            # img = Image.fromarray( img.transpose(1, 2, 0), 'L' )
...
class ImagePool(object):
    ...
    def add(self, imgs, cn, targets=None):
        # print("cn in imagepool: ", cn)
        if cn == 3:
            save_image_batch(imgs, os.path.join( self.root, "%d.png"%(self._idx) ), pack=False)
        elif cn == 1:
            save_image_batch_gray(imgs, os.path.join( self.root, "%d.png"%(self._idx) ), pack=False)
        self._idx+=1
